...
console.log('Трататаем products.js...');
class ProductManager {
    ...
    if (this.searchResults.length === 0) {
      ...
      resultsContainer.innerHTML = `
                ...
                <div class="search-results-grid">
                    ${this.searchResults
                      .slice(0, 6)
                      .map((product) => this.renderSearchResult(product))
                      .join('')}
                </div>
                ${
  ...
  renderSearchResult(product) {
    ...
    const badgeClass =
      product.status === 'new' ? 'new' : product.status === 'popular' ? 'popular' : product.discount > 0 ? 'sale' : '';
    ...
    return `
      ...
      product.name
    }" onerror="this.src='https://via.placeholder.com/250x350/F5F1FF/5A5A5A?text=No+Image'">
                </div>
                <div class="info">
                    <h4>${product.name}</h4>
  ...
  renderSectionProducts(selector, products) {
    ...
    container.innerHTML = sectionProducts.map((product) => this.renderProductCard(product)).join('');
  }
